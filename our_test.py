...
parser.add_argument('--save_dir', default='test_our_model_abl0', type=str,
...
def main():
    ...
    # load model
    our_model = load_our_model(args)
    our_model.eval()
    # reduce CPU usage, use it after the model is loaded onto the GPU
    torch.set_num_threads(1)
    cudnn.benchmark = True

    if os.path.exists("test_our_model_abl0") is False:
        os.mkdir("test_our_model_abl0")

    # start testing for all kinds of noises
    for i in range(0, len(noise_db)):

        args.save_dir = "test_our_model_abl0/" + noise_db[i][0] + "_" + noise_db[i][1]
        print(args.save_dir)
        ...
        for j in range(0, args.test_num):
            ...
            args.save_dir = "test_our_model_abl0/" + noise_db[i][0] + "_" + noise_db[i][1] + "/test" + str(j)
            if os.path.exists(args.save_dir) is False:
                os.mkdir(args.save_dir)
            ...
            arrive, flag, avg_infer_time = test_our_model(our_model, paths, labels, image_augment, args)
            ...
            with open("test_our_model_abl0/" + noise_db[i][0] + "_" + noise_db[i][1] + "/result.txt", "a") as file1:
...
def init_input(paths, labels, image_augment, args):
    ...
    args.start_path = [screenshot(paths, labels, 0, args.last_pos[0], args.last_pos[1], image_augment, -90, args)]

    # the end point image path which is masked to satisfy double-blind principle
    args.dest_path = "1,1.png"


def test_our_model(our_model, paths, labels, image_augment, args):
    """
    ...
    with torch.no_grad():
        for i in range(1, 250):
            # load model input
            images, next_angles = load_our_dataset(args)
            ...
            output1, output2, output3 = our_model(images, next_angles)
            ...
            lat_delta = args.dis * output3[0]
            lon_delta = args.dis * output3[1]
            ...
            path = screenshot(paths, labels, i, new_lat, new_lon, image_augment, ang, args)
            if path == -1:
                ...
                if len(args.start_path) > 5:
                    assert len(args.start_path) == len(args.next_angles) + 1
                    args.start_path.pop(0)
                    args.next_angles.pop(0)

                lat_diff = (new_lat - args.dest_pos[0]) * 111000
                lon_diff = (new_lon - args.dest_pos[1]) * 111000 * math.cos(args.dest_pos[0] / 180 * math.pi)
...
def screenshot(paths, labels, number, new_lat, new_lon, img_aug, ang, args):
    # new frame path
