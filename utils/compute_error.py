...
def get_success_rate():
    ...
    for i in range(0, len(noise_db)):
        ...
        _, one_noise_res = get_one_noise_res(path="../test_fsra_abl0",
    ...
    for i in range(0, len(all_noise_res)):
        ...
        print(int(all_noise_res[i][0]))
        print(int(all_noise_res[i][1]))
        print(int(all_noise_res[i][2]))
...
def get_MEPE():
    ...
    for i in range(0, 1):
        # get the result txt of a kind of noise
        one_noise_res, _ = get_one_noise_res(path="../test_our_model",
                                             noise=noise_db[i][0] + "_" + noise_db[i][1])
        all_noise_res.extend(one_noise_res)
...
def get_MRE():
    ...
    for i in range(0, 1):
        ...
        interp_lon, interp_lat = get_one_noise_interp_route(path="../test_our_model",
                                                            noise=noise_db[i][0] + "_" + noise_db[i][1])
...
def get_MRD():
    ...
    for i in range(0, len(all_interp_lat)):
        ...
        if diff is None:
            lon_diff = (lon_true[-1] - lon_true[0]) * 111000 * \
                       math.cos(interp_lat[0] / 180 * math.pi)
            lat_diff = (lat_true[-1] - lat_true[0]) * 111000
            # compute RD on the premise of not deviating from the prescribed route
            diff = math.sqrt(lon_diff * lon_diff + lat_diff * lat_diff)
            all_diff += diff
            total_num += 1
