...
parser.add_argument('--save_dir', default='test_rknet_abl0', type=str,
...
def main():
    ...
    if os.path.exists("test_rknet_abl0") is False:
        os.mkdir("test_rknet_abl0")

    # start testing for all kinds of noises
    for i in range(0, len(noise_db)):

        args.save_dir = "test_rknet_abl0/" + noise_db[i][0] + "_" + noise_db[i][1]
        print(args.save_dir)
        ...
        for j in range(0, args.test_num):
            ...
            args.save_dir = "test_rknet_abl0/" + noise_db[i][0] + "_" + noise_db[i][1] + "/test" + str(j)
            if os.path.exists(args.save_dir) is False:
                os.mkdir(args.save_dir)
            ...
            arrive, flag, avg_infer_time = test_match_model(match_model, route, paths, labels,
                                                            val_transform, image_augment, args)
            ...
            with open("test_rknet_abl0/" + noise_db[i][0] + "_" + noise_db[i][1] + "/result.txt", "a") as file1:
...
def get_most_match(model, val_transform, image_augment, feature, ideal_idx):
    """
    ...
    :return:
    """
    dir = "utils/candidates/" + str(ideal_idx)
    ...
    for candi in candidates:
        full_candi = os.path.join(dir, candi)
    ...
    candidates_feature, _, _ = model(candidates_input, None, None)
    ...
    # compare actual feature and candidate features: 1,dim&9,dim
    candidates_feature -= feature
    euclidean = torch.norm(candidates_feature, dim=-1).view(-1)

    # find the most match candidate image and return corresponding coordinates
    top1 = torch.argmax(euclidean)
    preds = candidates[top1]
    preds = preds[0:-5]
    preds = preds.split(',')
    preds = [float(preds[0]), float(preds[1])]

    return preds


def test_match_model(match_model, route, paths, labels, val_transform, image_augment, args):
    """
    ...
    with torch.no_grad():
        for i in range(1, 100):
            ...
            feature, _, _ = match_model(images, None, None)
            if len(feature.shape) is 3:
                ...
                feature = feature.div(fnorm.expand_as(feature))
            preds = get_most_match(match_model, val_transform, image_augment, feature, i - 1)
