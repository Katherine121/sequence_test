...
class rknet_three_view_net(nn.Module):
    def __init__(self, class_num, droprate, stride=1, pool='max', share_weight=False, VGG16=False):
        super(rknet_three_view_net, self).__init__()
        if VGG16:
            self.model_1 = ft_net_VGG16(class_num, stride=stride, pool=pool)
            self.model_2 = ft_net_VGG16(class_num, stride=stride, pool=pool)
        else:
